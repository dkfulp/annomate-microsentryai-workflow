...
HELP_ENTRIES = [
    ...
    HelpEntry(
        ...
        full_text=(
            "Open the Brush Thickness popup (◢ in the tool palette, just below "
            "the Polygon tool) and drag the slider to change the thickness used "
            "for new polygons. Select an existing polygon first if you want to "
            "change its outline width after the fact instead."
        ),
        object_name="toolBrushThicknessButton",
    ),
    HelpEntry(
        ...
        full_text=(
            ...
            "Esc to cancel. Use the ⚙ SAM Options button to switch model variants — "
            "the first use of a variant loads its weights, which can take a few "
            "seconds and is shown in the tool palette's status label."
        ),
    ...
    HelpEntry(
        ...
        full_text=(
            "Open SAM Options (⚙ in the tool palette, below the SAM tool) and "
            "pick a variant from the dropdown. The first use of a variant "
            "downloads and loads its weights, which can take a few seconds; "
            "progress is shown in the popup's status label."
        ),
        object_name="toolSamOptionsButton",
    ),
    HelpEntry(
        ...
        full_text=(
            "Select the calibrate tool (or press 'C') and click two points on the "
            "image that correspond to a known real-world distance — for example, "
            "the two ends of a ruler visible in the shot. A dialog then asks for "
            "that real distance and its unit (e.g. '5mm', '100um', '0.5in'). Once "
            "set, the measure tool and any exported measurements use this scale. "
            "The calibration controls live in the Grid Settings popup (⚙ in the "
            "viewport actions bar under the canvas)."
        ),
        object_name="viewportGridSettingsButton",
    ),
    HelpEntry(
        ...
        full_text=(
            "Select the measure tool (⇔ in the viewport actions bar under the "
            "canvas, or press 'M') and click two points on the image. The "
            "distance is reported in real-world units using the scale set by the "
            "calibration tool — calibrate the image first if you haven't "
            "already, otherwise the measurement has no unit to convert to."
        ),
        object_name="viewportMeasureButton",
    ),
    HelpEntry(
    ...
    HelpEntry(
        ...
        full_text=(
            "The center template tools (⌗ Center Crop in the viewport actions "
            "bar) let you mark the true center of a recurring frame or fixture. "
            "Start center calibration, click the center point, then accept it — "
            "the project must be saved first, since the template is stored "
            "alongside the project. Once saved, AnnoMate automatically matches "
            "this template against every subsequent image and re-centers the "
            "crop overlay to line up, without needing to recalibrate each image "
            "by hand. Use 'Clear Center Template' to remove it."
        ),
        object_name="viewportCenterCropButton",
    ),
    HelpEntry(
        ...
        full_text=(
            "The Anomaly Constraints popup (⊿ in the viewport actions bar) "
            "flags annotations that may need a closer look. Turn on 'Check "
            "Area' to highlight any polygon larger than a set maximum area, "
            "and/or 'Check Distance' to highlight pairs of annotations closer "
            "together than a set minimum distance (measured centroid-to-"
            "centroid or edge-to-edge). Violations are outlined directly on "
            "the canvas in the colors chosen here, and the counts shown next "
            "to each check update as you annotate. Thresholds and distances "
            "use the image's calibrated real-world units when one has been "
            "set, otherwise pixels."
        ),
        object_name="viewportAnomalyButton",
    ),
    HelpEntry(
        ...
        description="Select a polygon on the canvas or in the right panel list to edit, resize, or delete it.",
        full_text=(
            "Click a polygon on the canvas, or select it from the annotation list "
            "in the right panel, to select it — its thickness slider updates to "
            "match. Drag its points to reshape it, adjust the thickness slider to "
            "change its outline width, or press Delete to remove the selected "
            "annotation entirely."
        ),
        object_name="panelAnnotationsHeader",
    ),
    HelpEntry(
    ...
    HelpEntry(
        ...
        full_text=(
            "The Dataset Navigator panel on the right lists every image in the "
            "loaded folder, with a colored dot marking each image's review "
            "status (in review vs. reviewed). Use the ‹ Prev / Next › buttons or "
            "click a row to jump to that image, and use the Columns button to "
            "choose which metadata columns are shown in the list."
        ),
        object_name="panelNavigatorHeader",
    ),
    HelpEntry(
        ...
        full_text=(
            "The Annotation Classes panel on the right lists every class in the "
            "project. Type a name and click 'Add Class' to create a new one, or "
            "click a class in the list to make it the active class that new "
            "polygons (drawn or SAM-segmented) are assigned to."
        ),
        object_name="panelClassesHeader",
    ),
    HelpEntry(
    ...
    HelpEntry(
        ...
        description="Record who inspected each image and add a free-text note, in the Inspector/Notes panel.",
        full_text=(
            "The Inspector/Notes panel on the right holds an inspector name and "
            "a free-text note per image. Click 'Set Inspector' to save the "
            "current name as the session inspector, which then pre-fills for any "
            "image that doesn't already have one saved."
        ),
        object_name="panelInspectorHeader",
    ),
    HelpEntry(
    ...
    HelpEntry(
        ...
        full_text=(
            ...
            "P = polygon tool, S = SAM segment tool, C = calibrate tool, "
            "M = measure tool, Delete = delete the selected annotation. "
            "(A/D and the drawing-tool keys are disabled while center-crop "
            "calibration is in progress.) File shortcuts: Ctrl+N = New "
            "Project, Ctrl+O = Open Project, Ctrl+S = Save Project, "
            "Ctrl+Shift+S = Save Project As, Ctrl+Q = Exit."
        ),
    ),
    HelpEntry(
